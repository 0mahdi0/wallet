...
<?php if (!isset($_GET['action']) || !isset($_GET['item']) ) {
    ...
    <?php }  foreach($users as $user){
            $count_wallet = $wpdb->get_results("SELECT COUNT(`user_id`) AS TOTALCOUNT FROM {$wpdb->prefix}wallet_users WHERE `user_id` =".$user->ID." AND `status` = 2");
        ...
        <?php
            ...
            if ($_GET['item'] == $user->ID) {
                include(WPO_DIR."backend/wallet-users/wallet-list/wallet_show_Y.php");
            }
        }}}} ?>
        </form>
    <tr>
